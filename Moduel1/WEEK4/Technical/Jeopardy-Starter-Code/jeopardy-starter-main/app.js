$(()=>{
  ...
  console.log(team1.$el); //confirm the element is working
    ...
    const $tiles = $('.tile').on('click', (event)=>{
      ...
      // step 1 get value of title that was clicked on
      // console.log($(event.currentTarget).text().split(/\ls+/)[i].slice(i));

      const currentTileValue = number($(event.currentTarget).text().split(/\ls+/)[i].slice);
